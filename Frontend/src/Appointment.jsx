...
const Appointment = () => {
  ...
  // ── API-fetched doctor detail (includes availability, ratings, total patients) ──
  const [docDetail, setDocDetail] = useState(null)
  const [detailLoading, setDetailLoading] = useState(false)

  // ── Fetch full doctor detail from backend ──
  const fetchDocDetail = async (id) => {
    try {
      setDetailLoading(true)
      const { data } = await axios.get(`${backendUrl}/api/user/doctor/${id}`)
      if (data.success) setDocDetail(data.doctor)
    } catch (err) {
      console.log('Could not fetch doctor detail:', err.message)
    } finally {
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // If slot is now taken, refresh doctor data so the slot disappears
        if (data.message?.toLowerCase().includes('slot')) getDoctorsData()
      }
    } catch (error) {
      console.log(error)
  ...
  useEffect(() => { if (docInfo) { getAvailableSlots(); fetchDocDetail(docInfo._id) } }, [docInfo])

  // ── Derived values from API — fall back to docInfo if detail not yet loaded ──
  const doc = docDetail || docInfo   // docDetail has full fields; docInfo has basics
  const isAvailable = doc?.available ?? false
  const rating      = doc?.averageRating  ?? doc?.rating  ?? null
  const totalPatients = doc?.totalPatients ?? doc?.patients ?? null
  const satisfaction  = doc?.satisfaction  ?? null

  // Calculate today's available slots count from computed docSlots
  const todaySlotsCount = docSlots[0]?.length ?? 0
        ...
        <div className="ap-bg">
          ...
          <div className="ap-wave-wrap">
            <svg viewBox="0 0 1440 36" preserveAspectRatio="none" fill="none" style={{width:'100%',height:'100%'}}>
              ...
              <defs>
                <linearGradient id="wg"  x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#ec4899" stopOpacity=".7"/><stop offset="50%" stopColor="#a855f7" stopOpacity=".7"/><stop offset="100%" stopColor="#6366f1" stopOpacity=".7"/></linearGradient>
                <linearGradient id="wg2" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#6366f1" stopOpacity=".5"/><stop offset="50%" stopColor="#0ea5e9" stopOpacity=".5"/><stop offset="100%" stopColor="#ec4899" stopOpacity=".5"/></linearGradient>
              </defs>
            </svg>
          </div>
        </div>
        ...
        <div className="ap-inner">
          <div className="ap-hero">
            ...
            <div className="ap-img-card">
              ...
              {/* Availability tag — from API */}
              <div className={`ap-img-tag ${isAvailable ? 'ap-tag-avail-yes' : 'ap-tag-avail-no'}`}>
                <span className={isAvailable ? 'ap-avail-dot-on' : 'ap-avail-dot-off'} />
                {detailLoading ? 'Checking…' : isAvailable ? 'Available Today' : 'Not Available'}
              </div>

              {/* Rating tag — from API, hidden while loading */}
              {!detailLoading && rating !== null && (
                <div className="ap-img-tag ap-tag-rating">
                  ★ {typeof rating === 'number' ? rating.toFixed(1) : rating} · Top Rated
                </div>
              )}
              {detailLoading && (
                <div className="ap-img-tag ap-tag-rating">
                  <div className="ap-skel" style={{width:80,height:14}}/>
                </div>
              )}
            </div>
            ...
            <div className="ap-info-panel">
              ...
              <div className="ap-stats">
                {[
                  {
                    val: satisfaction !== null ? `${satisfaction}` : null,
                    suffix: '%',
                    label: 'Patient Satisfaction'
                  },
                  {
                    val: rating !== null ? `${typeof rating === 'number' ? rating.toFixed(1) : rating}` : null,
                    suffix: '★',
                    label: 'Average Rating'
                  },
                  {
                    val: totalPatients !== null
                      ? totalPatients >= 1000
                        ? `${(totalPatients / 1000).toFixed(1)}`
                        : `${totalPatients}`
                      : null,
                    suffix: totalPatients !== null && totalPatients >= 1000 ? 'k' : '+',
                    label: 'Total Patients'
                  },
                ].map((st, i) => (
                  <div className="ap-stat" key={i} style={{ animationDelay: `${0.3 + i * 0.12}s` }}>
                    {detailLoading || st.val === null ? (
                      <div className="ap-skel ap-stat-skel" />
                    ) : (
                      <div className="ap-stat-n">{st.val}<span>{st.suffix}</span></div>
                    )}
                    <div className="ap-stat-l">{st.label}</div>
                  </div>
                ))}
              </div>
          ...
          <div className="ap-booking">
            <div className="ap-booking-inner">
              ...
              {/* Unavailability notice — API driven */}
              {!detailLoading && !isAvailable && (
                <div className="ap-unavail-notice">
                  <span className="ap-unavail-icon">⚠️</span>
                  This doctor is currently not accepting new appointments. You can still browse available slots when they reopen.
                </div>
              )}

              {/* Days — slot count from computed docSlots */}
              <div className="ap-days">
                {docSlots.length > 0 && docSlots.map((item, index) => (
                  <div
                    key={index}
                    className={`ap-day ${slotIndex === index ? 'on' : ''}`}
                    onClick={() => { setSlotIndex(index); setSlotTime(''); setBookingError('') }}
                  >
                    ...
                    <span className="ap-day-slots">{item.length > 0 ? `${item.length} slots` : 'Full'}</span>
                  </div>
                ))}
              </div>
              ...
              {docSlots.length > 0 && docSlots[slotIndex]?.length === 0 ? (
                <p className="ap-no-slots">No slots available for this day. Please try another date.</p>
              ) : (
                <div className="ap-times">
                  {docSlots.length > 0 && docSlots[slotIndex]?.map((item, index) => (
                    <div
                      key={index}
                      className={`ap-chip ${item.time === slotTime ? 'on' : ''}`}
                      onClick={() => { setSlotTime(item.time); setBookingError('') }}
              ...
              {bookingError && (
                <div className="ap-book-error">
                  <span className="ap-book-error-icon">⚠️</span>
                  <div>
                    <div className="ap-book-error-msg">{bookingError}</div>
                    <div className="ap-book-error-sub">Please select a different time slot and try again.</div>
                  </div>
                </div>
              )}

              {/* Book button — disabled when doctor not available */}
              <button
                className={`ap-book-btn${booking ? ' loading' : ''}`}
                onClick={bookAppointment}
                disabled={!isAvailable || !slotTime || booking}
                title={!isAvailable ? 'Doctor is not accepting appointments' : !slotTime ? 'Select a time slot first' : ''}
              >
                {booking
